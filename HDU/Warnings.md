...
hash不仅有对字符串求hash值，它也是一种思想

它利用数组的下标来直接得到value与下标相同的个数，利用下标很重要。
...
我们主要讲第三种，比如下面的代码，如果输入“1 *”，if语句为假，不输出a,b，下一轮继续

但如果没scanf("*")，输入原来的会怎么样，答案是阻塞，因为输入\*时，if语句已经跳出，\*留在缓冲区内，这样&a永远读不到int类型的值，你可以用fflush(stdin);清空缓冲区，也可以用scanf("\*");读走。如果不输入\*也没事，因为读不到就跳出了这个语句。
...
\>=n的都是必败点，能一步到达必败点的是必胜点，只能到达必胜点的是必败点。
...
现在有n枚棋子，有k种取法，因此有k个不同的局面，这k个局面就是当前状态的后继，然后赋给sg[n]为后继中未出现的最小值。
...
这里的dp[n]\[3]: 0代表子树中最远的距离，1代表子树中第二远的距离，2代表该点经过父节点最远的距离。

在计算经过父节点的最远距离时，设子节点为son，父节点为r。

如果dp[son]\[0]+w==dp[r]\[0]，意味着dp[r]\[0]经过son，那么就要在第二远的地方找，则dp[son]\[2]=max(dp[r]\[1],dp[r]\[2])+w，否则dp[son]\[2]=max(dp[r]\[0],dp[r]\[2])+w
...
Floyd判断连通，bellman ford找正权回路和终点的最大值，SPFA这里其实不能用，因为如果有多个正权回路且第一个找出来的不符合就WA了，但这到题测试数据不强。
